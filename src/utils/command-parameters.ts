...
const TEMPERATURE_PATTERNS = [
  new RegExp(`(?:^|\\s)(?:--temperature|--temp)\\s*[:=]?\\s*${NUMBER_PATTERN}(?![\\d.])`, "i"),
  new RegExp(`(?:^|\\s)-t\\s*[:=]?\\s*${NUMBER_PATTERN}(?![\\d.])`, "i"),
  new RegExp(`(?:^|\\s)temperature\\s*[:=]?\\s*${NUMBER_PATTERN}(?![\\d.])`, "i"),
  new RegExp(`(?:^|\\s)temp\\s*[:=]?\\s*${NUMBER_PATTERN}(?![\\d.])`, "i"),
];
